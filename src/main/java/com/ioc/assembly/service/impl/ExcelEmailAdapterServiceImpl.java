 ...
 */
@Slf4j
@Service(value = ExcelConsts.EXCEL_VALIDATE_EMAIL_TYPE)
public class ExcelEmailAdapterServiceImpl implements ExcelAdapterService {
        ...
        for (int i = sheet.getFirstRowNum()+1; i <= sheet.getLastRowNum(); i++) {
            ...
            try {
                ...
                if(StringUtils.isNotEmpty(cellValue)){
                    if(!RegexUtil.match(RegexConsts.REG_EMAIL,cellValue)){
                        throw new RuntimeException( cellValue+"不是邮箱地址,校验规则不匹配！" );
                    }
                }

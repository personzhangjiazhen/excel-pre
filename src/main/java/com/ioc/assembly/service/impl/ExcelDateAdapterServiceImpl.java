...
@Slf4j
@Service(value = ExcelConsts.EXCEL_VALIDATE_DATE)
public class ExcelDateAdapterServiceImpl implements ExcelAdapterService {
        ...
        for (int i = sheet.getFirstRowNum()+1; i <= sheet.getLastRowNum(); i++) {
            ...
            try {
                ...
                if(StringUtils.isNotEmpty(cellValue)){
                    if(!RegexUtil.match(RegexConsts.REG_DATE,cellValue)){
                        throw new RuntimeException( cellValue+"不是日期,校验规则不匹配！" );
                    }
                }

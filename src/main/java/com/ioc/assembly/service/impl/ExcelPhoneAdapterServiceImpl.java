...
@Slf4j
@Service(value = ExcelConsts.EXCEL_VALIDATE_PHONE_TYPE)
public class ExcelPhoneAdapterServiceImpl implements ExcelAdapterService {
            ...
            try {
                ...
                if(StringUtils.isNotEmpty(cellValue)){
                    if(cellValue.length() != 11){
                        throw new RuntimeException( cellValue + "填入的手机号长度不正确，校验规则不匹配！" );
                    }else{
                        if(!RegexUtil.match(RegexConsts.REG_PHONE,cellValue)){
                            throw new RuntimeException( cellValue + "填入的不是手机号，校验规则不匹配！" );
                        }
                    }
                }
